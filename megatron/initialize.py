...
def _warmup_jit_function():
    ...
    bias = torch.rand(args.ffn_hidden_size // args.tensor_model_parallel_size,
                      dtype=dtype, device=get_accelerator().current_device_name())
    ...
    residual = torch.rand((seq_length // args.ds_sequence_parallel_size, args.micro_batch_size, args.hidden_size),
                          dtype=dtype, device=get_accelerator().current_device_name())
    bias = torch.rand((args.hidden_size), dtype=dtype, device=get_accelerator().current_device_name()).expand_as(residual)
    dropout_rate = 0.1
    # Warmup JIT fusions with the input grad_enable state of both forward
    # prop and recomputation
