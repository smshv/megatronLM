...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    if 'OMPI_COMM_WORLD_LOCAL_RANK' not in os.environ:
        print('OMPI_COMM_WORLD_LOCAL_RANK is not setup before model wrapped with ddp')
    else:
        print('OMPI_COMM_WORLD_LOCAL_RANK is setup before model wrapped with ddp')
    if wrap_with_ddp:
    ...
    if 'OMPI_COMM_WORLD_LOCAL_RANK' not in os.environ:
        print('OMPI_COMM_WORLD_LOCAL_RANK is not setup after model wrapped with ddp')
    else:
        print('OMPI_COMM_WORLD_LOCAL_RANK is setup after model wrapped with ddp')
...
def setup_model_and_optimizer(model_provider_func,
    ...
    if args.deepspeed:
        ...
        model = [model]
        print_rank_0('Deepspeed has been intitialized')
