...
do
    VISIBLE_DEVICES="${VISIBLE_DEVICES},${i}"
done
export CUDA_VISIBLE_DEVICES=$VISIBLE_DEVICES

#echo $VISIBLE_DEVICES
#export OMPI_COMM_WORLD_RANK=$LSB_JOBINDEX
#echo "Printing world rank: $OMPI_COMM_WORLD_RANK"

#deepspeed="/autofs/nccsopen-svm1_proj/gen150/smprof/bin/deepspeed"
###############################################################################
### Main configs
...
WARMUP_TOKENS=100 #375000000
# LR_DECAY_TOKENS=260000000000
LR_DECAY_TOKENS=100 #300000000000
###############################################################################
### Parallelism configs
...
if [ "${USE_INTERNAL_DATA}" = "true" ]; then
    ...
    DATA_PATH=./dataset/oscar-en-10k_text_document
...
run_cmd="python3 pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options}"
# &> ${OUTPUT_BASEPATH}/log/gptmoe.log"
#echo ${run_cmd}
eval ${run_cmd}
set +x
